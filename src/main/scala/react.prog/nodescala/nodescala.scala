package react.prog.nodescala

import com.sun.net.httpserver._
import react.prog.nodescala.nodescala.{CancellationToken, Subscription, CancellationTokenSource}
      ...
      Future {
        while (ct.nonCancelled) {
          listener.nextRequest() onComplete {
            case Success((req, exch)) => respond(exch, ct, handler(req))
            case Failure(exception)   => listenerSubscription.unsubscribe()
          }
        }
      }
    })
    val token = CancellationTokenSource.apply()
    token
